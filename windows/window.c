...
#define f(j,c,w,u,i) \
    ...
    if (kbd_codepage == 949) {
		...
		wfname[19] = L'\0';
	if (wcsncmp( wfname, L"나뭄", 2) == 0 ||
		wcsncmp( wfname, L"D2Coding", 8) == 0 ||
		wcsncmp( wfname, L"Noto Sans CJK", 13) == 0 ||
		wcsncmp( wfname, L"굴림", 2) == 0 ||
		wcsncmp( wfname, L"궁서", 2) == 0 ||
		wcsncmp( wfname, L"돋움", 2) == 0 ||
		wcsncmp( wfname, L"맑은", 2) == 0 ||
		wcsncmp( wfname, L"문체부", 3) == 0 ||
		wcsncmp( wfname, L"바탕", 2) == 0 ||
		wcsncmp( wfname, L"바탕", 3) == 0 ||
		wcsncmp( wfname, L"새굴림", 2) == 0 ||
		wcsncmp( wfname, L"휴먼", 2) == 0) {
